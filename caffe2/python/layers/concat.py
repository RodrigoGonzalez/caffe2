    ...
    def __init__(self, model, input_record, axis=1,
        ...
        if axis == 0:
            self.output_schema = schema.from_blob_list(
                input_record[0], [model.net.NextScopedBlob(f'{name}_output')]
            )
            return
        ...
        self.output_schema = schema.Scalar(
            (np.float32, output_dims), model.net.NextScopedBlob(f'{name}_output')
        )
    ...
    def add_ops(self, net):
        net.Concat(
            ...
            [
                ...
                f"{self.output_schema.field_blobs()[0]}_concat_dims",
            ],
            axis=self.axis,
        )
